...
// PackageName is required to render the result: RenderRegistry returns an
...
func fillAttributeData(data *RegistryData, schema *ParsedSchema, cfg RegistryConfig, attrIndex map[string]AttributeSpec, allAttrNames []string) {
	...
	for _, name := range allAttrNames {
		...
		// @regex/@range constraints round-trip through the registry so schema
		// intent is not silently lost (issue #77).
		if a.Regex != "" {
			data.AttrRegex = append(data.AttrRegex, KVCtx{name, a.Regex})
		}
		if a.RangeOp != "" {
			data.AttrRange = append(data.AttrRange, KVCtx{name, a.RangeOp})
		}
	}
...
// RenderRegistry writes a complete, gofmt-formatted schema registry Go file
// from RegistryData. It returns an error when data.PackageName is empty
// (the generated file would not be valid Go) or when the rendered output
// fails to format.
func RenderRegistry(w io.Writer, data *RegistryData) error {
	if data.PackageName == "" {
		return fmt.Errorf("tqlgen: RegistryConfig.PackageName is required")
	}
	return writeFormattedGo(w, registryTemplate, data)
}
...
// RenderLeafConstants writes a standalone, gofmt-formatted leaf package
// containing only type, relation, and enum constants. This package has zero
// internal dependencies, making it safe to import from any package. It
// returns an error when cfg.PackageName is empty.
func RenderLeafConstants(w io.Writer, schema *ParsedSchema, cfg LeafConstantsConfig) error {
	if cfg.PackageName == "" {
		return fmt.Errorf("tqlgen: LeafConstantsConfig.PackageName is required")
	}
	...
	return writeFormattedGo(w, leafTemplate, regData)
}
...
// AttributeRegex maps attribute name → @regex constraint pattern.
...
// AttributeRange maps attribute name → @range constraint expression.
