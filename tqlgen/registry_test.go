...
func TestRenderRegistry(t *testing.T) {
	schema := &ParsedSchema{
		...
		Entities:   []EntitySpec{{Name: "persona", Owns: []OwnsSpec{{Attribute: "name"}}}},
		Relations:  []RelationSpec{{Name: "acts", Relates: []RelatesSpec{{Role: "actor"}, {Role: "action"}}}},
	}
	data := BuildRegistryData(schema, RegistryConfig{PackageName: "graph", SkipAbstract: true})
	...
	out := buf.String()
	if !strings.Contains(out, "package graph") {
		t.Error("missing package declaration")
	}
	if !strings.Contains(out, `const TypePersona = "persona"`) {
		t.Error("missing entity constant")
	}
	if !strings.Contains(out, `const RelActs = "acts"`) {
		t.Error("missing relation constant")
	}
	if !strings.Contains(out, `"name": "string"`) {
		t.Error("missing attribute value type")
	}
}
