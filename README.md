...
```bash
go get github.com/CaliLuke/go-typeql@v1.14.1
```

The `ast/`, `gotype/`, and `tqlgen/` packages work without CGo or a running database. The `driver/` package targets TypeDB `3.12.1` with the `typedb-driver` and `typeql` `3.12.0` Rust crates. Stay on the `v1.10.x` line for TypeDB `3.10.x`.
...
gh release download v1.14.1 -p "libtypedb_go_ffi-${platform}.a" -R CaliLuke/go-typeql
