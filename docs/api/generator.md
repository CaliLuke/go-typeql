...
`@meta` annotations are parsed and preserved in `ParsedSchema`, but generated
ORM structs do not emit a general `typedb_meta` tag.
...
- `@doc`, `@meta` annotations (parsed; `@doc` is emitted for ORM structs)
