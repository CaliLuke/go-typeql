...
When parsing schema annotations, `tqlgen` decodes escaped string literals used in TypeQL string values, including unicode escapes such as `\u0041` and `\u{1F600}` in `@regex(...)`, `@values(...)`, `@doc(...)`, and `@meta(...)`.
...
- Go comments, `typedb_doc` tags, and `SchemaDoc()` methods from TypeDB `@doc`
  annotations
- Go comments and `SchemaMeta()` methods from type-level TypeDB `@meta`
  annotations; comments for capability-level `@meta` annotations
...
## TypeDB 3.12 Documentation Annotations

`tqlgen` preserves `@doc` and `@meta` in the public `ParsedSchema` model. For
generated ORM structs, `@doc` is emitted as Go documentation and as reflectable
metadata so `gotype.GenerateSchema` can recreate the annotation:

```typeql
entity customer @doc("A customer account."),
    owns email @key @doc("Primary contact email.");
```
...
type Customer struct {
    ...
    Email string `typedb:"email,key" typedb_doc:"Primary contact email."`
}

func (Customer) SchemaDoc() string { return "A customer account." }
```

`@meta` annotations are parsed, preserved in `ParsedSchema`, and rendered as
comments. Type-level `@meta` also generates `SchemaMeta()` so metadata can
round-trip through `gotype.GenerateSchema`:
...
func (Customer) SchemaMeta() map[string]string {
...
Generated ORM structs do not emit a general `typedb_meta` tag. Field metadata
remains parse/render-comment only.
...
- `@doc`, `@meta` annotations (`@doc` is emitted for ORM structs; `@meta` is
