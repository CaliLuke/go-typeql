...
| `lean/DiffMain.lean` | The `diffmodel` executable: runs the `Scopes.lean` model on filter trees as JSON, for the differential test in `gotype/compile_diff_test.go` | Lean 4 |
...
`lake build` checks all model files. `lake build diffmodel` builds the executable for the differential test. If the build completes with no error, all proofs are correct. The files use Lean core and `Std` only (no Mathlib).
