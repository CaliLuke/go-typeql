...
```bash
# Unit tests (401 tests, no DB or CGo needed)
...
Use `/release-checks <version>` to run the full 14-step release checklist (tests, coverage, `go vet`, `golangci-lint`, `staticcheck`, docs, Rust artifact verification, tagging, changelog, pkg.go.dev verification). See `.agents/skills/release-checks/skill.md`.
